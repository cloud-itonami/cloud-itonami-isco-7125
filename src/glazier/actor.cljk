(ns glazier.actor
  "GlazierActor — the ISCO-08 7125 glazing job-site
  scheduling/logistics coordination actor as a
  `langgraph.graph/state-graph` (ADR-2607121000 / CLAUDE.md Actors
  section). One graph run = one job-site-coordination operation
  ...
  resume after human sign-off. Modeled on cloud-itonami-isco-7111's
  housebuilder.actor for the physical-safety-domain shape.
  ...
  The unconditional invariant: the Glazier Advisor can never directly
  commit an operating record the GlazierGovernor refuses, and can
  never make this actor finalize a glazing-installation-execution
  decision (e.g. deciding to proceed with a specific glass-panel
  installation) or override a site safety officer's judgment — every
  ...
  never performs glazing-installation work or authorizes glazing work
  itself."
...
(defn build-graph
  "Build a compiled GlazierActor graph. `store` implements
  `glazier.store/Store`. `advisor` implements `glazier.advisor/Advisor`
  (defaults to `mock-advisor`). `checkpointer` defaults to an
  in-memory one."
  ...
  (-> (g/state-graph
      ...
      (g/add-node :commit
                   (fn [{:keys [request proposal]}]
                     (let [record {:glazier-id (:glazier-id request)
                                    :op (:op proposal)
