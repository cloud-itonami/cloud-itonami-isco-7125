(ns glazier.governor-test
  (:require [clojure.test :refer [deftest is testing]]
            [glazier.store :as store]
            [glazier.advisor :as advisor]
            [glazier.governor :as governor]))
...
(def ^:private req {:glazier-id "glazier-1"})
...
(deftest hard-on-unregistered-glazier
  (let [st (fresh-store)
        v (governor/check {:glazier-id "nobody"} {} (op :log-work-record) st)]
    (is (:hard? v))
    (is (some #(= :no-glazier (:rule %)) (:violations v)))))
...
(deftest hard-on-scope-excluded-op-finalize-glazing-installation-decision
  (testing "finalizing a glazing-installation-execution decision is a permanent block, never a routine op"
    (let [st (fresh-store)
          v (governor/check req {} (assoc (op :finalize-glazing-installation-decision) :confidence 0.99) st)]
      (is (:hard? v))
      (is (some #(= :scope-excluded-action (:rule %)) (:violations v))))))

(deftest hard-on-scope-excluded-op-authorize-panel-installation
  (testing "authorizing a glass-panel installation is a permanent block, never a routine op"
    (let [st (fresh-store)
          v (governor/check req {} (assoc (op :authorize-panel-installation) :confidence 0.99) st)]
      (is (:hard? v))
      (is (some #(= :scope-excluded-action (:rule %)) (:violations v))))))

(deftest hard-on-scope-excluded-op-proceed-with-glazing-installation
  (testing "proceeding with the glazing installation itself is a permanent block, never a routine op"
    (let [st (fresh-store)
          v (governor/check req {} (assoc (op :proceed-with-glazing-installation) :confidence 0.99) st)]
      (is (:hard? v))
      (is (some #(= :scope-excluded-action (:rule %)) (:violations v))))))
...
(deftest hard-on-scope-excluded-op-override-site-safety-officer-judgment
  (testing "overriding a site safety officer's judgment (fully-qualified op form) is also a permanent block"
    (let [st (fresh-store)
...
(deftest hard-on-scope-excluded-rationale-even-with-allowlisted-op
  (testing "defense-in-depth: a rationale that itself attempts to proceed with the glazing installation is blocked even if the op looks routine"
    (let [st (fresh-store)
          v (governor/check req {} (assoc (op :schedule-crew-operation)
                                           :rationale "recommend we proceed with the glazing installation now")
                             st)]
      (is (:hard? v))
      (is (some #(= :scope-excluded-action (:rule %)) (:violations v))))))
...
(deftest always-escalates-safety-concern-even-at-high-confidence
  (testing "a glass-handling/height-work/panel-hazard concern always requires human sign-off"
    (let [st (fresh-store)
          v (governor/check req {} (assoc (op :flag-safety-concern :hazard-type :glass-handling-risk)
                                           :confidence 0.99)
                             st)]
      (is (not (:hard? v)))
      (is (:escalate? v)))))
...
(deftest default-mock-advisor-proposals-never-self-trip-on-scope-exclusion
  (testing "the governor's scope-exclusion term list must never match the mock advisor's own default rationale text for any allowlisted op — CLAUDE.md's known self-tripping bug pattern (rationale legitimately contains bare nouns like 'glass'/'panel'/'safety officer', but never the full finalization-action phrases)"
    (let [st (fresh-store)
          adv (advisor/mock-advisor)
          ops [:log-work-record :schedule-crew-operation
               :flag-safety-concern :coordinate-supply-order]]
      (doseq [o ops]
        (let [request {:glazier-id "glazier-1" :op o :site-id "S-1"
                        :stake :low :task "routine glazing task"
                        :hazard-type :glass-handling-risk :cost 500}
